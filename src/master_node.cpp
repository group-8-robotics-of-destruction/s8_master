...
private:
    ...
    void explore_done_callback(const actionlib::SimpleClientGoalState& state, const s8_explorer::ExploreResultConstPtr & result) {
        ...
        if(result->reason == ExploreFinishedReason::REVISITED) {
            ...
            goal.type = s8::mapper_node::NavigateType::ToClosestUnexplored;
            navigate_action.sendGoal(goal);
            navigate_action.waitForResult(ros::Duration(ACTION_NAVIGATE_TIMEOUT));
            auto result = navigate_action.getResult();
            ROS_INFO("Done %d", result->reason);
            if(result->reason == 2) {
                //Everything explored! Go to root?
                ROS_INFO("Going to root.");
                goal.type = s8::mapper_node::NavigateType::ToRoot;
                navigate_action.sendGoal(goal);
                navigate_action.waitForResult(ros::Duration(ACTION_NAVIGATE_TIMEOUT));

                result = navigate_action.getResult();
                if(result->reason == 4) {
                   ROS_INFO("ALL DONE");
                   exit(0);
               }
            ...
            start_explore();
        } else if(result->reason == ExploreFinishedReason::PREEMPTED) {
        ...
        if(object_distance > 0 && object_size >= 100 && object_distance <= 0.65) {
            ...
            if(object_detected_in_row_count > 1 && (current_time-classify_time).toSec() > 3) {
                if(exploring) {
                    ...
                    ROS_INFO("Object detected! Will stop exploring and then classify.");
                    stop_exploring();
                } else {
                    if(!aligned) {
                        //classify = false;
